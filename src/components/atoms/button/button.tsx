...
const Button: FC<ButtonProps> = ({ size, label, classes, onClick, variant, children }) => {
  ...
  return (
    <button className={`${defaultClasses} - ${classes} - ${currentSize} - ${variant === 'purple' ? purpleVariant : variant === 'colorful' ? colorfulVariant : defaultVariant}`}
            onClick={onClick}>
